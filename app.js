...
class QuizApp {
  ...
  loadStateFromStorage() {
    try {
      const storedProgress = localStorage.getItem('it_review_progress');
      if (storedProgress) {
        this.state.quizProgress = JSON.parse(storedProgress);
      }

      const storedFavorites = localStorage.getItem('it_review_favorites');
      if (storedFavorites) {
  ...
  saveStateToStorage() {
    try {
      localStorage.setItem('it_review_progress', JSON.stringify(this.state.quizProgress));
      localStorage.setItem('it_review_favorites', JSON.stringify(this.state.favorites));
  ...
  async loadQuestionBank() {
    try {
      ...
      console.info('Direct fetch of questions.csv failed (likely opened via file:// protocol). Showing local file selector modal.', error);
  ...
  processCSVData(csvText) {
    ...
    if (questions.length === 0) {
      showGlassAlert('Could not parse any valid questions from questions.csv. Please verify file structure.', 'Parsing Error', '⚠️');
      return;
    }
