...
class QuizApp {
  ...
  loadStateFromStorage() {
    try {
      const storedProgress = localStorage.getItem('ism302_review_progress');
      if (storedProgress) {
        this.state.quizProgress = JSON.parse(storedProgress);
      }

      const storedFavorites = localStorage.getItem('ism302_review_favorites');
      if (storedFavorites) {
  ...
  saveStateToStorage() {
    try {
      localStorage.setItem('ism302_review_progress', JSON.stringify(this.state.quizProgress));
      localStorage.setItem('ism302_review_favorites', JSON.stringify(this.state.favorites));
  ...
  async loadQuestionBank() {
    try {
      let response = await fetch('questions_ism.csv');
      if (!response.ok) {
        response = await fetch('ism302/questions_ism.csv');
      }
      if (!response.ok) {
      ...
      console.info('Direct fetch of questions_ism.csv failed (likely opened via file:// protocol or CORS). Showing local file selector modal.', error);
  ...
  processCSVData(csvText) {
    ...
    if (questions.length === 0) {
      showGlassAlert('Could not parse any valid questions from questions_ism.csv. Please verify file structure.', 'Parsing Error', '⚠️');
      return;
    }
